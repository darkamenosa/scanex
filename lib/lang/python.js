...
export function scan(src, { file }) {
  ...
  try {
    ...
    // Show what imports were found for the original posts.py file
    if (file.includes('posts.py') && imports.length > 0) {
      console.log(`[codesnap] Found ${imports.length} Python imports in ${file.split('/').pop()}: ${imports.join(', ')}`);
    }
...
export function resolve(spec, { file, projectRoot }) {
  ...
  if (spec.startsWith('.')) {
    return resolveRelativeImport(spec, file, projectRoot);
  }
  
  // Handle absolute imports
  return resolveAbsoluteImport(spec, file, projectRoot);
}
