...
class ScanexMCPServer {
  ...
  async analyzeCodebase(args) {
    ...
    // Parse inputs - resolve relative to current working directory if needed
    const INPUTS = input.split(',').map(p => {
      const trimmed = p.trim();
      const resolved = resolve(trimmed);
      // If the resolved path doesn't exist, try relative to cwd
  ...
  async scanDependencies(args) {
    ...
    // Resolve file path properly - it might be relative to current working directory
    const resolvedFile = resolve(inputFile);
    
    let actualFile;
    if (!existsSync(resolvedFile)) {
      // If the resolved path doesn't exist, try relative to current working directory
      const cwdRelativeFile = resolve(process.cwd(), inputFile);
      if (!existsSync(cwdRelativeFile)) {
        throw new Error(`File does not exist: ${inputFile} (tried: ${resolvedFile} and ${cwdRelativeFile})`);
      }
      // Use the working directory relative path
      actualFile = cwdRelativeFile;
    } else {
      actualFile = resolvedFile;
    }
    ...
    return {
      content: [
        {
          type: 'text',
          text: `Dependencies found in ${relative(projectRoot, actualFile)}:\n\n${dependencies.map(dep => 
  ...
  async generateTree(args) {
    ...
    // Resolve input path properly - it might be relative to current working directory
    let inputPath = resolve(input);
    if (!existsSync(inputPath)) {
      // Try relative to current working directory
      const cwdRelativePath = resolve(process.cwd(), input);
      if (!existsSync(cwdRelativePath)) {
        throw new Error(`Path does not exist: ${input} (tried: ${inputPath} and ${cwdRelativePath})`);
      }
