...
class ScanexMCPServer {
  ...
  async scanDependencies(args) {
    ...
    // Smart path resolution with project directory detection
    let actualFile = null;
    ...
    if (inputFile.startsWith('/')) {
      ...
      // Strategy 2: Try relative to current working directory (unless it's clearly wrong)
      if (cwd !== '/' && !cwd.includes('/tmp') && !cwd.includes('/var/')) {
        const cwdRelative = resolve(cwd, inputFile);
        if (existsSync(cwdRelative)) {
          actualFile = cwdRelative;
        }
      }
      
      // Strategy 3: If working directory seems wrong, try to find project directory
      if (!actualFile && (cwd === '/' || cwd.includes('/tmp') || cwd.includes('/var/'))) {
        // Look for common project patterns in the file path
        // e.g., if file is "app/controllers/foo.rb", look for directories containing "app"
            ...
            if (existsSync(baseDir)) {
              try {
                ...
                for (const dir of dirs) {
                  ...
                  const testPath = join(projectPath, inputFile);
                  if (existsSync(testPath)) {
                    actualFile = testPath;
                    console.error(`ℹ️  Auto-detected project directory: ${projectPath}`);
                    break;
                  }
                }
                if (actualFile) break;
      ...
      // Strategy 4: Try absolute resolution as fallback
      if (!actualFile) {
    ...
    if (!actualFile) {
      const attempts = [
        resolve(cwd, inputFile),
        resolve(inputFile)
      ];
      
      // Add project detection attempts to error message
      if (cwd === '/' || cwd.includes('/tmp') || cwd.includes('/var/')) {
        throw new Error(`File does not exist: ${inputFile}

❌ MCP server is running from system directory: ${cwd}
This prevents proper path resolution.

🔧 Solutions:
1. Add "cwd": "." to your Cursor MCP configuration
2. Or use absolute file paths like: /full/path/to/your/project/${inputFile}

Tried paths:
${attempts.map(p => `- ${p}`).join('\n')}
...
Current working directory: ${cwd}`);
  ...
  async run() {
    ...
    // Warn if starting from suspicious directories
    const cwd = process.cwd();
    if (cwd === '/' || cwd === '/tmp' || cwd.includes('/var/')) {
      console.error('⚠️  Warning: MCP server started from system directory.');
      console.error('   This may cause path resolution issues.');
      console.error('   Consider adding "cwd": "." to your MCP configuration.');
    }
  }
}
